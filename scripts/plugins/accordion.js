    ...
    $.extend( $.summernote.plugins, {
        ...
        'accordion': function (context) {
			...
			this.moveSectionUp = function(){
				...
				if(previousSection) {
					let newNode = currentSection.parentNode.insertBefore(currentSection, previousSection);
					  const ph = $(this.nodes[0]).get(0).parentElement;
					  const pos = $(ph).offset();
			...
			this.moveSectionDown = function(){
				...
				if(nextSection) {
					let newNode = currentSection.parentNode.insertBefore(nextSection, currentSection);
					  const ph = $(this.nodes[0]).get(0).parentElement;
					  const pos = $(ph).offset();
					  const height = $(ph).outerHeight(true); // include margin
					  this.$popover.css({
			...
			this.addSection = function() {
				...
				newHTML += '       <button class="btn btn-link" type="button" data-toggle="collapse" data-parent="#' + accordionContainer.id + '" data-target="#' + newCollapseId + '" href="#' + newCollapseId + '" aria-expanded="true" aria-controls="' + newCollapseId + '">';
				newHTML += '         New Section';
				newHTML += '       </button>';
				newHTML += '     </h4>';
				newHTML += '   </div>';
				...
				$('#'+newHeadingId).find('.btn').last().trigger('click');
			}
			...
			this.removeSection = function(){
				...
				if($(accordionContainer).find('.card').length == 0) {
					...
					if(previousSection) {
						$(previousSection).find('.card-header .btn').last().trigger('click');				
					} else if(nextSection) {
						$(nextSection).find('.card-header .btn').last().trigger('click');				
					}
				}
			}		
              ...
              this.showAccordionDialog().then(function (data) {
                ...
                if (data.length > 0) {
				  ...
				  if(nodes.length > 1) {
					  ...
					  //TODO: Only do this for a single node..probably need to disable accordion if more than 1 node selected
					  nodes[0].parentNode.insertBefore(div, nodes[0].nextElementSibling);
				  } else {
					  //Let summernote insert the node, summernote will split/close nodes to ensure 
					  //HTML makes sense. This is good, but it does not work well inside a tab content panel
					  //as it actually splits the tab container and inserts the node
            ...
            this.showAccordionDialog = function () {
              return $.Deferred(function (deferred) {
                ...
                ui.onDialogHidden(self.$dialog, function () {
                  // Remove panel
                  var $accordionPanel = self.$dialog.find('.card');
