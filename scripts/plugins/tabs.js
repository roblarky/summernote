    ...
    $.extend($.summernote.plugins, {
        ...
        "tabs": function(context){
			...
			this.moveTabLeft = function(){
				let currentTab = $(this.nodes[0]).closest('.nav-item').get(0); 
				let previousTab = currentTab.previousElementSibling;
				if(previousTab) {
					let newNode = currentTab.parentNode.insertBefore(currentTab, previousTab);
			...
			this.moveTabRight = function(){
				...
				if(nextTab) {
					let newNode = currentTab.parentNode.insertBefore(nextTab, currentTab);
					  const ph = $(this.nodes[0]).get(0).parentElement;
					  const pos = $(ph).offset();
					  const height = $(ph).outerHeight(true); // include margin
					  this.$popover.css({
			...
			this.addTab = function() {
				...
				let tabContainer = $(currentTab).closest('.tabs-container').get(0);
				...
				newTabContent.classList.add('fade');
			...
			this.removeTab = function(){
				...
				if($(tabContainer).find('.nav-item').length == 0) {
					if($(tabContainer).closest('.tabs-block')) {
						$(tabContainer).closest('.tabs-block').remove();
					} else {
					...
					if(previousTab) {
						$(previousTab).find('.nav-link').last().trigger('click');						
					} else if(nextTab) {
						$(nextTab).find('.nav-link').last().trigger('click');						
					}					
				}
			}
			...
			this.showDialog = function(){
              ...
              this.showTabsDialog().then(function (data) {
                ...
                if (data.length > 0) {
				  ...
				  if(nodes.length > 1) {
					  ...
					  //TODO: Only do this for a single node..probably need to disable tab if more than 1 node selected
					  nodes[0].parentNode.insertBefore(div, nodes[0].nextEleSibling);
				  } else {
